...
test('system settings control optional mailbox workspace entries', async ({ page }) => {
  ...
  const naverMailSwitch = settings.getByRole('checkbox', { name: 'NAVER 邮箱入口' })
  ...
  await expect(naverMailSwitch).toBeChecked()
  ...
  await naverMailSwitch.uncheck()
  await expect.poll(() => state.naverMailWorkspaceEnabled).toBe(false)
  await expect(page.getByRole('button', { name: 'NAVER 邮箱' })).toHaveCount(0)
  ...
  await settings.getByRole('checkbox', { name: 'NAVER 邮箱入口' }).check()
  ...
  await expect(page.getByRole('button', { name: 'NAVER 邮箱' })).toBeVisible()
